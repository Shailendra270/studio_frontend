...
import { Star, ChevronLeft, ChevronRight, Pause, Play, Volume2, VolumeX, Plus, X } from 'lucide-react';
import moment from 'moment';

interface WebStoriesPlayerProps {
  clips: any[];
}

const WebStoriesPlayer: React.FC<WebStoriesPlayerProps> = ({ clips }) => {
  ...
  const getEditedVideoForAspect = (clip: any, aspect = '9:16') => {
    return clip?.editedVideos?.find(
      (editedVideo: any) =>
        editedVideo.aspect_ratio === aspect &&
        editedVideo.event !== 'DYNAMIC'
    );
  };

  const hasNineSixteen = (clip: any) => {
    return (
      clip?.aspectRatio === '9:16' ||
      (clip?.editedVideos?.some(
        (ev: any) => ev.aspect_ratio === '9:16' && ev.event !== 'DYNAMIC'
      ) ?? false)
    );
  };

  const getNineSixteenVideoUrl = (clip: any) => {
    const ev = getEditedVideoForAspect(clip, '9:16');
    return ev?.videoUrl || (clip?.aspectRatio === '9:16' ? clip?.videoUrl : undefined);
  };

  const getPosterUrl = (clip: any) => {
    const ev = getEditedVideoForAspect(clip, '9:16');
    return ev?.thumbnailUrl || clip?.thumbnailUrl || clip?.thumbnail;
  };

  const getAspectRatioDisplay = (clip: any) => {
    const ev = getEditedVideoForAspect(clip, '9:16');
    return ev?.aspect_ratio || clip?.aspectRatio;
  };
  ...
  const getClipTime = (clip: any) => {
    ...
    if (clip?.date) {
      const formatted = `${clip.date}${clip?.time ? ` ${clip.time}` : ''}`;
  ...
  const displayClips = clips
    .filter(hasNineSixteen)
    .slice()
    .sort((a, b) => getClipTime(a) - getClipTime(b));

  const currentClip = displayClips[currentClipIndex];
  ...
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    const url = getNineSixteenVideoUrl(currentClip);
    if (url) {
      video.src = url;
      video.load();
      setProgress(0);
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <p className="text-gray-400">Add some clips to view them as web stories.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex items-center justify-center bg-[#18191B] relative">
      {/* Main Video Container */}
      <div className="relative">
        {/* Video Player Container - 9:16 aspect ratio */}
        <div className="w-[377px] h-[670px] bg-[#252525] rounded-xl overflow-hidden relative">
          {/* Progress Bars */}
          ...
          <video
            ...
            poster={getPosterUrl(currentClip)}
            muted={isMuted}
            playsInline
            autoPlay
          />
      ...
      <div className="w-[270px] ml-8 space-y-6">
        {/* Clip Title and Rating */}
        <div>
          <h2 className="text-xl font-bold text-white mb-3">{currentClip.title}</h2>
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="px-3 py-1 bg-black rounded-md">
              <span className="text-white text-xs">{getAspectRatioDisplay(currentClip)}</span>
            </div>

            {/* Duration */}
            <div className="px-3 py-1 bg-black rounded-md">
              <span className="text-white text-xs">{moment.utc(currentClip.duration * 1000).format('mm:ss')}</span>
            </div>
          </div>

          {/* Tags */}
          <div className="flex flex-wrap gap-2 mb-6">
            {currentClip.tags.map((tag, index) => (
              <div key={index} className="flex items-center gap-2 px-3 py-2 border border-white rounded-lg">
...
export default WebStoriesPlayer;
