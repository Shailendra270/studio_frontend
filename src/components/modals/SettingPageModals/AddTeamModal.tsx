...
import { getTags, TagData, importPlayersFromDSG } from "@/api/tagsApi";
...
const AddTeamModal: React.FC<AddTeamModalProps> = ({ isOpen, onClose, onSubmit, defaultSport }) => {
  const dispatch = useDispatch();
  const user = useSelector(selectUser);
  const [sport, setSport] = useState<string>(defaultSport || "football");
  const [teamName, setTeamName] = useState<string>("");
  const [selectedPlayerIds, setSelectedPlayerIds] = useState<string[]>([]);
  // Entity ID removed per requirement
  ...
  const [externalTeamId, setExternalTeamId] = useState<string>("");
  // const streams = useSelector((state: any) => state.streams.streams);

  useEffect(() => {
    setSport(defaultSport || "football");
  }, [defaultSport]);

  // Removed entityId autofill logic
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (!sport) return;
      try {
        console.log("Fetching player tags for sport:", sport);
        const resp = await getTags({ category: sport, tagType: "player", userId: user?.userId || '', limit: 20 });
  ...
  // Fetch players by search when not found locally
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
  ...
  const playerOptions = useMemo(() => {
    ...
    for (const o of [...selectedOptions, ...baseOpts]) map.set(String(o.value), String(o.label));
  ...
  }, [playerTags, sport, selectedPlayerIds, selectedOptions]);

  // Entity ID options removed
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[630px] bg-black border-2 border-[#373737] rounded-[50px] z-10">
        ...
        <div className="relative px-8 py-8 border-b border-[#373737]">
          <h2 className="text-[28px] font-medium text-white text-center font-montserrat">Add team</h2>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <Input
                ...
                inputMode="numeric"
                maxLength={5}
                className="flex-1 h-[50px] px-4 bg-[#252525] border-none rounded-xl text-white text-base font-medium placeholder-[#707070] focus:outline-none focus:ring-2 focus:ring-[#00BBFF]"
              />
              <Input
              ...
              <Button
                onClick={async () => {
                  ...
                  const t = String(externalTeamId).trim();
                  if (!/^\d{5}$/.test(s)) {
                    toast.error("Enter a valid 5-digit Season ID");
                    return;
                  }
                  if (!t) {
                  ...
                  const p = importPlayersFromDSG({ teamId: t, seasonId: s, category: sport, userId: user?.userId || '', createTeam: true });
                  toast.promise(p, {
                    loading: "Syncing team players…",
                    success: (r) => r?.message || "Team players synced",
                    error: (e) => e?.message || "Unauthorized or invalid Season/Team ID",
                  });
                  handleClose();
                  try {
                    await p;
                    const resp = await getTags({ category: sport, tagType: "player", userId: user?.userId || '', limit: 100 });
                    if (resp.success) setPlayerTags(resp.data || []);
                    dispatch<any>(fetchTeams({ category: sport, limit: 10, pageNo: 1, userId: user?.userId || '' }));
                  } catch {}
                }}
                className="w-[100px] h-[50px] border-[1.5px] border-white rounded-xl text-white text-base font-medium transition-colors bg-[#1B1B1B] hover:bg-[#252525]"
              >
                Sync
              </Button>
            </div>
